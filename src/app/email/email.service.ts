...
export class EmailService {
  ...
  constructor(
    @Inject('EMAIL_CONFIG') private readonly mailerConfig: EmailModuleOptions,
  ) {
    if (
      !mailerConfig.transport ||
      Object.keys(mailerConfig.transport).length < 1
    ) {
      throw new Error(
  ...
  async sendMail(
    sendMailOptions: SendMailOptions & EmailTemplate,
